...
pub fn export_chart_png(app: &App, path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption(
            "Tempest Monitor — Performance History",
    ...
    chart
        .draw_series(LineSeries::new(cpu_data, &BLUE))?
        .label("CPU %")
        .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], BLUE));

    let mem_data: Vec<(i32, i32)> = app
        .ram_history
        .iter()
        .enumerate()
        .map(|(i, &v)| (i as i32, v as i32))
    ...
    chart
        .draw_series(LineSeries::new(mem_data, &MAGENTA))?
        .label("MEM %")
