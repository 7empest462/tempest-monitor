...
fn render_gpu_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let text = format!(
        " Model: {} │ GPU: {}{}{}{} ",
        model_str, usage_str, freq_str, pkg_str, vram_str
    );
...
fn power_gauge(label: &str, mw: Option<f64>, max_w: f64) -> Line<'static> {
    match mw {
        Some(mw_val) => {
            ...
            let bar_len = 20usize;
            let filled = ((watts / max_w) * bar_len as f64)
                .round()
                .clamp(0.0, bar_len as f64) as usize;
            let bar = format!("[{}{}]", "█".repeat(filled), "░".repeat(bar_len - filled));
            let color = if watts / max_w > 0.75 {
                Color::Rgb(243, 139, 168)
            } else if watts / max_w > 0.45 {
...
fn render_macos_power_panel(f: &mut Frame, app: &App, area: Rect) {
    let has_data =
        app.gpu_power_mw.is_some() || app.cpu_power_mw.is_some() || app.pkg_power_mw.is_some();
    ...
    let lines: Vec<Line> = if has_data {
        ...
        vec![
            ...
            Line::from(""),
            Line::from(Span::styled(
                " Run with sudo to enable",
                Style::default().fg(Color::Yellow),
            )),
            Line::from(Span::styled(
                " powermetrics readings.",
                Style::default().fg(Color::Yellow),
            )),
        ]
    };
...
fn render_nvidia_panel(f: &mut Frame, app: &App, area: Rect) {
    ...
    for gpu in &app.nvidia_gpus {
        ...
        items.push(ListItem::new(format!(
            " Power:      {:.2}W",
            gpu.power_usage_mw as f64 / 1000.0
        )));
        items.push(ListItem::new(format!(
            " Gfx Clock:  {}MHz",
...
fn render_linux_gpu_stats(f: &mut Frame, app: &App, area: Rect) {
    ...
    if lines.len() <= 2 {
        lines = vec![
            ...
            Line::from(""),
            Line::from(Span::styled(
                " Check /sys/class/drm/ and",
                Style::default().fg(Color::Yellow),
            )),
            Line::from(Span::styled(
                " /sys/class/hwmon/ for data.",
                Style::default().fg(Color::Yellow),
            )),
        ];
    }
    ...
    pub fn gpu_utilization() -> Option<f64> {
        GPU_HANDLES.with(|cell| {
            unsafe {
                ...
                let mut total = if sum_3d_like > 0.0 {
                    sum_3d_like
                } else {
                    sum_all_engines
                };
        ...
        MEM_HANDLES.with(|cell| {
            unsafe {
                ...
                if opt.is_none() {
                    ...
                    if PdhAddEnglishCounterW(query, PCWSTR(path_usage.as_ptr()), 0, &mut usage) != 0
                    {
                        return None;
                    }
                    if PdhAddEnglishCounterW(query, PCWSTR(path_limit.as_ptr()), 0, &mut limit) != 0
                    {
                        return None;
                    }
